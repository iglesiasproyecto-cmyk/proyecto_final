...
import { Plus, Trash2, Loader2, BookOpen, Settings, HelpCircle, Calendar } from 'lucide-react'
import { useForm, useFieldArray } from 'react-hook-form'
import { supabase } from '@/lib/supabaseClient'
import { toast } from 'sonner'
import { motion, AnimatePresence } from 'motion/react'

interface EditarEvaluacionDialogProps {
  open: boolean
...
export function EditarEvaluacionDialog({ open, onOpenChange, idEvaluacion, onSuccess }: EditarEvaluacionDialogProps) {
  ...
  useEffect(() => {
    if (!open || !idEvaluacion) return
    setLoadingData(true)

    supabase.rpc('get_evaluacion_detalle', { p_id_evaluacion: idEvaluacion })
      .then(({ data, error }) => {
        if (error || !data) {
        ...
        form.reset({
          ...
          intentos_permitidos: ev.max_intentos || 3,
          puntaje_minimo_aprobacion: ev.puntaje_minimo || 70,
          preguntas: (ev.preguntas || []).map((p: any) => {
            const tipoMapeado =
              p.tipo === 'opcion_multiple' ? 'multiple_choice' :
              p.tipo === 'verdadero_falso' ? 'verdadero_falso' : 'respuesta_corta'

            const opciones = (p.opciones || []).map((o: any) => o.texto)
            const correctaIdx = (p.opciones || []).findIndex((o: any) => o.es_correcta)
            
            let respuesta_correcta = ''
            if (tipoMapeado === 'multiple_choice') {
              respuesta_correcta = correctaIdx >= 0 ? String.fromCharCode(65 + correctaIdx) : 'A'
            } else if (tipoMapeado === 'verdadero_falso') {
              const opCorrecta = (p.opciones || []).find((o: any) => o.es_correcta)
              respuesta_correcta = opCorrecta?.texto || 'Verdadero'
            } else {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const preguntas = data.preguntas.map((pregunta, i) => {
        const tipoMapeado =
          pregunta.tipo === 'multiple_choice' ? 'opcion_multiple' :
          pregunta.tipo === 'verdadero_falso' ? 'verdadero_falso' : 'respuesta_corta'
      ...
      const { error } = await supabase.rpc('actualizar_evaluacion_completa', {
      ...
      <DialogContent className="max-w-[850px] max-h-[90vh] overflow-hidden flex flex-col p-0 rounded-3xl bg-card border border-border/80 backdrop-blur-2xl shadow-2xl">
        <DialogHeader className="p-6 pb-4 border-b border-border/40 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-xl font-black uppercase tracking-tight italic">Editar Evaluación</DialogTitle>
              <DialogDescription className="text-xs text-muted-foreground mt-0.5">
                Modifica los parámetros de evaluación, las preguntas del módulo y sus respuestas correctas.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        {loadingData ? (
          <div className="flex flex-col items-center justify-center py-20 gap-3 text-muted-foreground">
            ...
            <span className="text-xs font-bold uppercase tracking-widest">Cargando evaluación...</span>
          </div>
        ) : (
          <Form {...form}>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input className="rounded-xl border-border bg-background" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea
                            placeholder="Instrucciones para los servidores..."
                  ...
                  <div className="flex items-center justify-between border-b border-border/40 pb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-black uppercase tracking-widest text-muted-foreground">
                        Preguntas Configuradas ({fields.length})
                      </span>
                    </div>
                    <Button
                      type="button"
                              ...
                              <CardContent className="p-5 space-y-4">
                                ...
                                {tipoPregunta === 'multiple_choice' && (
                                  <div className="space-y-3 p-4 rounded-2xl bg-muted/10 border border-border/30">
                                            ...
                                            render={({ field }) => (
                                              <FormItem className="space-y-1.5">
                                                <div className="flex items-center gap-2">
                                                  ...
                                                  <FormControl>
                                                    <Input placeholder={`Opción ${letra}`} className="rounded-xl border-border bg-background h-9 text-xs" {...field} />
                                                  </FormControl>
                                                </div>
                                                <FormMessage />
                                              </FormItem>
                                    ...
                                    {/* RESPUESTA CORRECTA INTERACTIVA */}
                                  ...
                                  <div className="p-4 rounded-2xl bg-muted/10 border border-border/30 space-y-2">
                                    ...
                                    <FormField
                                      ...
                                      render={({ field }) => (
                                        <FormItem>
                                          <FormControl>
                                            <Input className="rounded-xl border-border bg-background text-xs" {...field} />
                                          </FormControl>
                                          <FormMessage />
                                        </FormItem>
                                      )}
