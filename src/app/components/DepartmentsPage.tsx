...
import { Users, Plus, Search, Settings, Power, PowerOff, BookOpen, UserCog, UsersRound, Inbox } from "lucide-react";
...
export function DepartmentsPage() {
  const { user, ministerios, miembrosMinisterio, toggleMinisterioEstado } = useApp();
  ...
  const filtered = churchMins.filter((m) => m.nombre.toLowerCase().includes(search.toLowerCase()));
  const min = selectedMin ? ministerios.find((m) => m.idMinisterio === selectedMin) : null;
  const minMembers = selectedMin ? miembrosMinisterio.filter((mm) => mm.idMinisterio === selectedMin) : [];

  if (selectedMin && min) {
    return (
      <div className="space-y-6 max-w-7xl mx-auto">
        <div className="flex items-center gap-3">
          <button onClick={() => setSelectedMin(null)} className="text-primary hover:underline text-sm">&larr; Ministerios</button>
        </div>
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filtered.map((min, i) => (
          <motion.div key={min.idMinisterio} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}>
            <Card className={`p-5 cursor-pointer hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 ${min.estado !== "activo" ? "opacity-60" : ""}`} onClick={() => setSelectedMin(min.idMinisterio)}>
              <div className="flex items-start justify-between mb-3">
                ...
                <Badge variant={min.estado === "activo" ? "default" : "secondary"} className="text-[10px]">{min.estado === "activo" ? "Activo" : "Inactivo"}</Badge>
              </div>
              <h3 className="mb-1">{min.nombre}</h3>
              <p className="text-sm text-muted-foreground mb-3 line-clamp-2">{min.descripcion}</p>
              <div className="flex items-center justify-between text-sm">
                <div className="flex items-center gap-1 text-muted-foreground"><UserCog className="w-3.5 h-3.5" /><span className="text-xs">{min.liderNombre}</span></div>
                <div className="flex items-center gap-1 text-muted-foreground"><UsersRound className="w-3.5 h-3.5" /><span className="text-xs">{min.cantidadMiembros}</span></div>
              </div>
              <div className="mt-3 pt-3 border-t flex gap-2">
                ...
                <Button variant={min.estado === "activo" ? "ghost" : "default"} size="sm" onClick={(e) => { e.stopPropagation(); toggleMinisterioEstado(min.idMinisterio); }}>
                  {min.estado === "activo" ? <PowerOff className="w-4 h-4" /> : <Power className="w-4 h-4" />}
                </Button>
              </div>
            </Card>
          </motion.div>
