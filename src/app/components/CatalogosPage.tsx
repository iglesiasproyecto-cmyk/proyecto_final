...
export function CatalogosPage() {
  const { roles, tiposEvento, addTipoEvento, updateTipoEvento, deleteTipoEvento, usuarios, usuarioRoles } = useApp();
  const [tab, setTab] = useState("tipos_evento");
  const [dialogTE, setDialogTE] = useState(false);
  const [editingTE, setEditingTE] = useState<string | null>(null);
  const [formTE, setFormTE] = useState({ nombre: "", descripcion: "" });
  const [confirmDeleteTE, setConfirmDeleteTE] = useState<{ id: string; name: string } | null>(null);

  const openAddTE = () => { setFormTE({ nombre: "", descripcion: "" }); setEditingTE(null); setDialogTE(true); };
  const openEditTE = (id: string) => {
  ...
  const getUserCountForRol = (idRol: string) => {
    return new Set(usuarioRoles.filter(ur => ur.idRol === idRol && !ur.fechaFin).map(ur => ur.idUsuario)).size;
  };
      ...
      <Tabs value={tab} onValueChange={setTab}>
        ...
        <TabsContent value="roles" className="space-y-4 mt-4">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {roles.map(r => {
              const count = getUserCountForRol(r.idRol);
              return (
                <Card key={r.idRol} className="p-4">
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    <Badge variant="secondary" className="text-xs">{count} usuario{count !== 1 ? "s" : ""} asignado{count !== 1 ? "s" : ""}</Badge>
