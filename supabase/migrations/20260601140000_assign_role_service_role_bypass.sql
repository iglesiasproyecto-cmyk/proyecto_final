-- complete-invite calls assign_role_with_ministerio via service_role (the invitee has
-- no user session yet when accepting an invite). The RPC's internal can_assign_role_scoped()
-- check returns false under service_role (no JWT context), so the role was silently never
-- assigned (the error is caught as non-fatal in complete-invite) — the invitee ended up
-- with no ministerio. service_role is trusted infrastructure and the invite token was
-- already validated, so allow service_role to bypass the internal scoped check.
...
BEGIN
  -- service_role (trusted edge functions like complete-invite) bypasses the scoped check.
  IF coalesce(auth.role(), '') <> 'service_role'
     AND NOT public.can_assign_role_scoped(p_id_rol, p_id_iglesia, p_id_sede, p_id_ministerio) THEN
